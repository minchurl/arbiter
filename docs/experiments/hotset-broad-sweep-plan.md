...
executes the exact `awk` row-count and ranking-format expressions used after
every row and phase. Runtime parse failures are counted, written to the
manifest, stop promotion, and make the final status
`controller-parse-error`. Promotion lists are also rejected unless every entry
has the expected `raw-NNN` shape and maps to a known built candidate. These
checks cover both preliminary-run failures: an ambiguous awk ternary in v1 and
a missing ranking CSV separator in v2.
...
tmux new-session -d -s hotset-broad-v2 \
  "cd /home/shin/arbiter && exec env OVERNIGHT_END_DEADLINE='tomorrow 09:00' ./scripts/run-hotset-broad-sweep-overnight.sh --run"
```
