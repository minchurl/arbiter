...
RAW_CANDIDATES="${HOTSET_RAW_CANDIDATES:-80}"
MAX_UNIQUE_CANDIDATES="${HOTSET_MAX_UNIQUE_CANDIDATES:-60}"
SEARCH_SEED="${HOTSET_SEARCH_SEED:-20260819}"
TOP_CONFIRM="${HOTSET_TOP_CONFIRM:-10}"
TOP_FINAL="${HOTSET_TOP_FINAL:-3}"
CONFIRM_EXTRA_PAIRS="${HOTSET_CONFIRM_EXTRA_PAIRS:-2}"
FINAL_EXTRA_PAIRS="${HOTSET_FINAL_EXTRA_PAIRS:-7}"
...
BUILD_BUDGET_SECONDS="${HOTSET_BUILD_BUDGET_SECONDS:-240}"
...
RESULT_DIR="${RESULT_DIR:-${ROOT_DIR}/build/arbiter-bench/hotset-broad-sweep-${RUN_ID}}"
...
Default adaptive schedule:
  generate/build:           80 raw configurations
  static de-duplication:    selected-site fingerprint, at most 60 candidates
  screen:                   one 15s local/CXL pair per candidate
  confirmation:             top 10, two additional 60s pairs (3 total)
  final:                    top 3, seven additional 180s pairs (10 total)
...
The controller also de-duplicates candidates by the allocation-site IDs that
actually execute during the screen.  A row with no arena report is retained as
a valid no-activity observation, but it cannot be promoted.

Useful overrides:
  OVERNIGHT_END_DEADLINE       optional GNU date string, e.g. 'tomorrow 09:00'
  HOTSET_SEARCH_SEED           deterministic search seed
  HOTSET_RAW_CANDIDATES        default 80
  HOTSET_MAX_UNIQUE_CANDIDATES default 60
...
if [[ "${RAW_CANDIDATES}" -lt 12 ]]; then
  echo "HOTSET_RAW_CANDIDATES must be at least 12 so all designed anchors are included" >&2
  exit 1
fi
...
# Designed, pin-free reference points come first.  Their order deliberately
...
next_candidate; MIN_SCORE=6; SEED_LIMIT=6; LARGE_THRESHOLD=96; DYNAMIC_ESTIMATE=96; MAX_SITES=24; MIN_AFFINITY=1; emit_candidate dynamic-small-estimate
...
dynamic_estimates=(96 128 512 1024 4096 16384)
byte_budgets=(0 0 1024 4096 16384 65536)
member_caps=(1 2 4 6 8)
affinities=(1 2 3 5 7)
call_depths=(0 1 2 3)
...
while [[ "${RAW_INDEX}" -lt "${RAW_CANDIDATES}" ]]; do
  ...
  pick_from dynamic_choices; INCLUDE_DYNAMIC="${PICKED}"
  ...
  pick_from dynamic_estimates; DYNAMIC_ESTIMATE="${PICKED}"
...
echo "[build] selected ${SELECTED_COUNT} static-unique candidates; build failures=${BUILD_FAILURES}"
printf 'selected_static_unique=%s\nbuild_failures=%s\n' "${SELECTED_COUNT}" "${BUILD_FAILURES}" >>"${MANIFEST}"
...
run_row() {
  ...
  if [[ -s "${row_dir}/runs.csv" ]]; then
    row_count="$(awk 'END {print NR > 0 ? NR - 1 : 0}' "${row_dir}/runs.csv")"
  fi
  ...
  if [[ -s "${row_dir}/throughput-samples.csv" ]]; then
    awk -v OFS=, \
      ...
      ' "${row_dir}/throughput-samples.csv" >>"${ALL_SAMPLES}"
  fi
...
write_phase1_ranking() {
  awk -F, 'NR > 1 && $2 == 1 && !seen[$9]++ {print $1 "," $8 "," $9}' "${RESULT_DIR}/phase1-summary.csv" \
    | sort -t, -k2,2nr -k1,1 >"${RESULT_DIR}/phase1-ranked.tmp"
  {
...
write_repeated_ranking() {
  ...
  awk -F, 'NR > 1 && $2 == 1 {print $1 "," $9 "," $12}' "${summary}" \
    | sort -t, -k2,2nr -k1,1 >"${output}.tmp"
  {
...
finalize() {
  ...
  FINALIZED=1
  if [[ -s "${OBSERVATIONS}" ]]; then write_native_summary; fi
  printf 'finished_at=%s\nfinal_status=%s\nrows_recorded=%s\n' \
    "$(date '+%F %T %Z')" "${final_status}" "$(awk 'END {print NR > 0 ? NR-1 : 0}' "${OBSERVATIONS}")" >>"${MANIFEST}"
  cat <<EOF
...
echo "[phase] confirmation: ${#CONFIRM_IDS[@]} runtime-unique candidates"
...
write_repeated_ranking "${RESULT_DIR}/phase2-summary.csv" "${RESULT_DIR}/phase2-ranking.csv" "${TOP_FINAL}" "${RESULT_DIR}/promoted-final.txt"
...
write_repeated_ranking "${RESULT_DIR}/final-summary.csv" "${RESULT_DIR}/final-ranking.csv" "${TOP_FINAL}" "${RESULT_DIR}/finalists.txt"
