...
#include "../libs/pebs/pebs.h"
#include "../libs/pebs/interpose.h"
...
static void *do_gups(void *arguments)
{
  ...
  for (i = 0; i < args->iters; i++) {
    ...
    if (hot_num < 90) {
      lfsr = lfsr_fast(lfsr);
      lfsr2 = lfsr_fast(lfsr2);
      index1 = ((lfsr % args->hot_start) * (2*1024*1024/8)) + (lfsr2 % args->hotsize);
      if (elt_size == 8) {
        uint64_t  tmp = field[index1];
        tmp = tmp + i;
        field[index1] = tmp;
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < threads; i++) {
    ...
    //ga[i]->field = p + i * ((uint64_t)1 << 30);//hardcode for 8 thread, 8GB hotset, optimal case.
    ga[i]->iters = updates;
    ga[i]->size = nelems;
    //ga[i]->size = ((uint64_t)1 << 30) / elt_size;
    ga[i]->elt_size = elt_size;
    ga[i]->hot_start = (nelems * elt_size) / (2*1024*1024);        // number of hot sectors
    ga[i]->hotsize = 256*1024 / elt_size;
