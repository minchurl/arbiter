...
for tool in cmake ninja clang clang++ llvm-config opt llvm-link llvm-dis; do
...
Optional MLIR legacy build:
  cmake -S "${ROOT_DIR}" -B "${ROOT_DIR}/build-llvm${LLVM_VERSION}-mlir" -G Ninja \\
    -DCMAKE_C_COMPILER="${LLVM_PREFIX}/bin/clang" \\
    -DCMAKE_CXX_COMPILER="${LLVM_PREFIX}/bin/clang++" \\
    -DARBITER_ENABLE_MLIR_LEGACY=ON \\
    -DMLIR_DIR="${LLVM_PREFIX}/lib/cmake/mlir"
  cmake --build "${ROOT_DIR}/build-llvm${LLVM_VERSION}-mlir" --target \\
    arbiter-opt arbiter_runtime_mlir_legacy
  ARBITER_BUILD_DIR="${ROOT_DIR}/build-llvm${LLVM_VERSION}-mlir" \\
    "${ROOT_DIR}/scripts/smoke-mlir-legacy.sh"
EOF
