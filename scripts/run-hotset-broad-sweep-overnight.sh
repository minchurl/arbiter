...
# Earlier controllers exposed two awk portability/formatting bugs only after
# hours of benchmark work.  Exercise the exact row-count and ranking-format
# expressions during check mode so promotion cannot silently become empty or
# produce malformed candidate IDs.
...
if ! AWK_RANK_SELF_TEST="$(printf 'raw-021,103.529,68+69+71+74\n' | \
    awk -F, -v OFS=, '{print NR,$1,$2,$3}')"; then
  echo "awk cannot execute the controller ranking expression" >&2
  exit 1
fi
if [[ "${AWK_RANK_SELF_TEST}" != '1,raw-021,103.529,68+69+71+74' ]]; then
  echo "awk ranking self-test returned malformed CSV: ${AWK_RANK_SELF_TEST}" >&2
  exit 1
fi
...
validate_promotion_file() {
  local file="$1" phase="$2" ranked_file="$3" candidate invalid=0
  if [[ -s "${ranked_file}" && ! -s "${file}" ]]; then
    echo "[controller-error] ${phase} ranking is nonempty but its promotion list is empty" >&2
    CONTROLLER_PARSE_FAILURES=$((CONTROLLER_PARSE_FAILURES + 1))
    return 1
  fi
  while IFS= read -r candidate || [[ -n "${candidate}" ]]; do
    if [[ ! "${candidate}" =~ ^raw-[0-9]{3}$ ]]; then
      echo "[controller-error] malformed ${phase} candidate ID: ${candidate:-<empty>}" >&2
      invalid=1
    elif [[ -z "${C_CONFIG[${candidate}]+present}" || \
            -z "${C_BUILD[${candidate}]+present}" || \
            -z "${C_STATIC[${candidate}]+present}" ]]; then
      echo "[controller-error] unknown ${phase} candidate ID: ${candidate}" >&2
      invalid=1
    fi
  done <"${file}"
...
write_phase1_summary
write_phase1_ranking
validate_promotion_file "${RESULT_DIR}/promoted-confirm.txt" confirmation \
  "${RESULT_DIR}/phase1-ranked.tmp" || true
abort_on_controller_parse_failure
...
validate_promotion_file "${RESULT_DIR}/promoted-final.txt" final \
  "${RESULT_DIR}/phase2-ranking.csv.tmp" || true
abort_on_controller_parse_failure
